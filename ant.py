...
class TD3Agent:
    ...
    def print_loss(self, replay_buffer, batch_size, discount):
        with torch.no_grad():
            ...
            # Compute actor loss
            actor_loss = -self.critic1_target(state, self.actor_target(state)).mean().item()

            print(f"Average actor prediction magnitude: {self.actor_target(state).abs().mean().item()}")
...
def main():
    ...
    while total_timesteps < MAX_TIMESTEPS:
        ...
        if iter_count >= (REPLAY_BUFFER_BATCH_SIZE // NUM_ENVS) * TRAINING_BEGIN and iter_count % TRAIN_FREQ == 0:
            ...
            print(f"Reward (3000): {avg_reward_3000:.2f} | Reward (Ep): {avg_reward_episode:.2f} | Episodes: {total_episodes} | Buffer size: {replay_buffer.size:.3e}")
